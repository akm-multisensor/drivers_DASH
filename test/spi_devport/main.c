...
void print_usage(const char *prog)
{
    ...
    fprintf(stdout, " -b bus : spi bus number.\n");
    fprintf(stdout, " -c cs  : spi chips select.\n");
}
    ...
    char **argv)
{
    ...
    /* try to open spi bus */
    sprintf(spi_dev_fn, "/dev/spidev%d.%d", g_bus, g_cs);

    if ((fd = open(spi_dev_fn, O_RDWR)) < 0) {
        err = errno;
        fprintf(stderr, "\nFailed to open spi port (%s).\n", spi_dev_fn);
        fprintf(stderr, " err=%d: %s.\n\n", err, strerror(err));
        return 1;
    }
